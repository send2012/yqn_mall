	...
	<div class="order_list">
        ...
        >
            <div>
                ...
                <div class="order_list--panel_inner_normal" :ref="'cardList' + i">
                    <van-card
                        class="order_list--van-card"
                        @click.native="toOrderDetail(item.pro_pid)"
                    >
                        <div slot="thumb">
                        ...
                        <div slot="desc" class="card_desc">
                            ...
                            <span class="card_desc_bz text-ellipsis">{{item.bz}}</span>
                            <span class="card_desc_spec_title text-ellipsis">{{item.spec_title}}</span>
                            <div class="card_desc_price">
                                <span class="card_desc_price_value">￥{{item.pricebuy}}</span>
        ...
        <div class="order-detail-count">
            <van-cell-group>
                ...
                <van-cell v-if="item.zt == 1 || item.zt == 3">
                    <template slot="title">
                        <span class="van-cell-text_right">
                                <component 
                                    slot="footer" 
...
<script>
    import { ORDER_GOODS_RESALE_INFO, ORDER_GOODS_RESALE_CHANGESTATUS } from '@/api/order';
	...
	export default {
		...
		methods: {
			initData() {
                ...
                this.$reqGet(ORDER_GOODS_RESALE_INFO,{goods: i}).then(res => {
                    this.item = Object.assign({},res.data);
                })
				// const status = this.tabsItem[i].status;
				// return this.$reqGet(ORDER_GOODS_RESALE_INFO, {
