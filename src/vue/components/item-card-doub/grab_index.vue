<template>
	...
	<div class="item_card_D_wrap">
		<div class="item_card_image" @click="OnClick">
            ...
            <!-- 左上角大促狂欢购 -->
            <!-- <img :src="require('../../../assets/images/'+ getImgExtra('promotion') +'.png')" alt="" class="item_card_image_pro" v-if="goods.prom_limit && goods.prom_limit === 2"/> -->
            ...
            <!-- 右下角秒杀直降 -->
            <span class="item_card_image_spike" v-if="goods.prom_limit">
                <span class="item_card_image_spike_text">距{{goods.limit_type === 1?'秒杀':'直降'}}结束：</span>
                    <countdown :time="getCountDownTime(goods.limit_enddate)">
		...
		<div :class="goods.point?'item_point_name':'item_card_name'" @click="OnClick">
			...
			<span class="goods_title_tag" v-if="goods.owner === 0">自营</span> {{goods.title + ' ' + goods.subtitle + ' ' + goods.spec}}
		</div>
        <div v-if="$slots.goodDesc" class="item_card_action">
                <!-- 商品详情 -->
				<slot name="goodDesc"></slot>
		</div>
        <div v-if="!goods.point">
            <!-- 非积分，普通商品形态 -->
            ...
            <div class="item_card_proDesc">
                <span class="item_card_proDesc_price">￥{{(goods.prom_limit.max_price === goods.prom_limit.min_price)?goods.prom_limit.max_price:(goods.prom_limit.min_price + '~' + goods.prom_limit.max_price)}}</span>
            </div>
		...
		<div class="item_card_action">
			<div class="item_card_action_point">
				...
				<span class="item_card_price_title">原价：{{goods.max_price}}</span>
			</div>
			<van-button type="warning" size="mini" @click="exchange">立即抢购</van-button>
		</div>
        ...
        methods: {
            ...
            getCountDownTime(stamp) {
                ...
                if(stamp) {
                    ...
                    let countDownTime = (stamp - nowStamp/1000) * 1000;
